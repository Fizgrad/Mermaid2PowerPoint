import type { BoundingBox, ParsedPathGeometry, PathCommandPx, PointPx } from "./types.js";

const TOKEN_PATTERN = /[AaCcHhLlMmQqSsTtVvZz]|-?(?:\d+\.\d+|\d+|\.\d+)(?:e[-+]?\d+)?/g;

export function parseSvgPathData(d: string | undefined): ParsedPathGeometry | undefined {
  if (!d) {
    return undefined;
  }
  ...
  const commands: PathCommandPx[] = [];
  const samples: PointPx[] = [];
  let index = 0;
  let command = "";
  let current = { x: 0, y: 0 };
  let subpathStart = { x: 0, y: 0 };
  let previousCubicControl: PointPx | undefined;
  let previousQuadraticControl: PointPx | undefined;

  const hasNumberAt = (tokenIndex: number): boolean =>
    tokenIndex < tokens.length && /^-?(?:\d+\.\d+|\d+|\.\d+)(?:e[-+]?\d+)?$/i.test(tokens[tokenIndex]);

  const readNumber = (): number => Number.parseFloat(tokens[index++]);
  const readPoint = (relative: boolean): PointPx => {
    const x = readNumber();
    const y = readNumber();
    return relative ? { x: current.x + x, y: current.y + y } : { x, y };
  };

  const pushSample = (...points: PointPx[]): void => {
...
export function geometryToPoints(geometry: ParsedPathGeometry | undefined): PointPx[] {
  if (!geometry) {
    return [];
  }
...
export function unionBoundingBoxes(boxes: BoundingBox[]): BoundingBox | undefined {
  if (boxes.length === 0) {
    return undefined;
  }
...
function getBoundsFromPoints(points: PointPx[]): BoundingBox {
  if (points.length === 0) {
    return {
      x: 0,
      y: 0,
